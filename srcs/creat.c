...
{
    char *dst;
    dst = data->image.addr + (y * data->image.line_length + x * (data->image.bpp / 8));
...
void	init_game(t_data *data)
{
	...
	data->image.addr = mlx_get_data_addr(data->image.img, &data->image.bpp, &data->image.line_length, &data->image.endian);
	if (!data->image.addr)
    {
		perror("Error\n");
